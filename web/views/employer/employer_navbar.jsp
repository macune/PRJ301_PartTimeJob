<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<div class="actor-navbar mb-4">
    <div class="container actor-navbar-scroll">
        <a href="${pageContext.request.contextPath}/employerDashboard" class="actor-nav-item nav-employer">
            <i class="fas fa-chart-line me-2"></i>Tổng quan
        </a>
        <a href="#" class="actor-nav-item nav-employer">
            <i class="fas fa-briefcase me-2"></i>Quản lý bài đăng
        </a>
        <a href="#" class="actor-nav-item nav-employer">
            <i class="fas fa-users me-2"></i>Quản lý ứng viên
        </a>
        <a href="#" class="actor-nav-item nav-employer">
            <i class="fas fa-store me-2"></i>Hồ sơ cửa hàng
        </a>
    </div>
</div>